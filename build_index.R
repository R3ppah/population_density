...
# 1. 读取孟加拉国边界
bd_admin <- st_read(file.path(ROOT_PATH, "Data", "Bangladesh_boundaries_20230628.gpkg")) %>%
  st_transform(3106)

# 2. 简化边界（减少计算复杂度）
bd_admin_simple <- st_simplify(bd_admin, dTolerance = 500, preserveTopology = TRUE)
...
if (length(results) > 0) {
  ...
  # 可视化
  plot(st_geometry(bd_admin_simple), main = "孟加拉国人口六边形")
